...
export const sessionApi = {
  ...
  // joinUrl / shareText: link http(s) de invitación y mensaje ya armado para compartir (EDT 1.1.1.1).
  create: (payload: { name: string; hostName: string; items: string[] }) =>
    httpClient.post<Session & { hostId: string; joinUrl: string; shareText: string }>(
      "/sessions",
      payload,
    ),
