...
export function NewSessionScreen() {
  ...
  async function handleCreate(shareViaWhatsApp: boolean) {
    ...
    try {
      ...
      if (shareViaWhatsApp) {
        // El mensaje ya viene armado del servidor con el link http(s) de invitación: WhatsApp no
        // vuelve clickeable un esquema propio (cuentasclaras://), así que compartirlo directo dejaba
        // el link como texto plano. /join/:token hace el puente al deep link (EDT 1.1.1.1).
        await shareSessionLinkViaWhatsApp(session.shareText);
      }
